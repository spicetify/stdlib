...
export const useSearchBar = ({ placeholder, expanded })=>{
    ...
    const searchbar = /*#__PURE__*/ React.createElement(FilterContext.Provider, {
        value: searchProps
...
export const useChipFilter = (filters)=>{
    ...
    const selectedFilters = React.useMemo(()=>selectedFilterFullKey.split(".").slice(1).reduce((selectedFilters, selectedFilterFullKeyPart)=>{
    ...
    const availableFilters = new Array();
    ...
    const treeNodeHasVal = (n)=>!!n.filter[TreeNodeVal];
    const chipFilter = /*#__PURE__*/ React.createElement(ChipFilter, {
